...
public final class ClaySoldierCookingRecipe {
    ...
    public static <T extends AbstractCookingRecipe> List<RecipeHolder<T>> createCookingRecipe(CookingRecipeBuilder<T> factory, int smeltingTime) {
        ...
        HolderSet<ItemStack> items = HolderSet.direct(
                ClayMobTeamManger.getAllKeys(Minecraft.getInstance().level.registryAccess())
        );
                ;
        Ingredient input = Ingredient.of(
                ClayMobTeamManger.getAllKeys(Minecraft.getInstance().level.registryAccess())
        ...
        return List.of();//List.of(new RecipeHolder<>(id, cookingRecipe));
    }
