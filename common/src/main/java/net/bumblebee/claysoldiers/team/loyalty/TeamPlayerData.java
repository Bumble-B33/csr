...
public interface TeamPlayerData {
     ...
     */
    boolean putPlayerIfAbsent(ResourceKey<ClayMobTeam> teamId, Player player);
    /**
     * @return the {@code Player} the given team is loyal to.
     */
    @Nullable
    PlayerData getPlayerForTeam(ResourceKey<ClayMobTeam> teamId);
     ...
     */
    void forEach(BiConsumer<ResourceKey<ClayMobTeam>, PlayerData> action);

    /**
     * Updates the player name of the {@code ClayMobTeam} that is loyal to the given {@code Player}.
     */
    void updatePlayerName(ResourceKey<ClayMobTeam> teamId, Player player);
