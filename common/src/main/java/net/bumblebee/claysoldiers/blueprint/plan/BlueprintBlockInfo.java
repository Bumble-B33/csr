...
public class BlueprintBlockInfo {
    ...
    public BlueprintBlockInfo(BlockPos pos, BlockState state, @Nullable CompoundTag nbt) {
        ...
        this.nbt = nbt;
        if (state.isAir()) {
            throw new IllegalStateException("State cannot be Air");
        }
    }
    ...
    public boolean isFor(ItemStack stack) {
        return BlueprintUtil.isItemForState(stack, state);
    }
     ...
     */
    public BlueprintRequest getRequest(long gameTime, BlockPos destinationPos, boolean getLast) {
        if (getLast && lastRequest != null) {
            return lastRequest;
        }
        var req = new BlueprintRequest(BlueprintUtil.itemFromBlockState(state), destinationPos, gameTime);
