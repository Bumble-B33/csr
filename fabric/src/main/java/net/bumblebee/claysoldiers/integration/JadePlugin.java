...
public class JadePlugin implements IWailaPlugin {
    ...
    public void registerClient(IWailaClientRegistration registration) {
        JadeRegistry.getBlocks().forEach(jadeBlock -> registration.registerBlockComponent(new FabricBlockProvider(jadeBlock), jadeBlock.getTargetClass()));
        JadeRegistry.getEntities().forEach(jadeEntity -> {
            var provider = new FabricEntityProvider<>(jadeEntity);
    ...
    public void register(IWailaCommonRegistration registration) {
        JadeRegistry.getServerEntities().forEach(jadeEntityServer -> {
            registration.registerEntityDataProvider(new FabricEntityDataProvider<>(jadeEntityServer), jadeEntityServer.getTargetClass());
        });
    }
    ...
    private record FabricEntityProvider<T extends Entity>(CommonEntityProvider<T> provider) implements IEntityComponentProvider {
        ...
        public void appendTooltip(ITooltip iTooltip, EntityAccessor entityAccessor, IPluginConfig iPluginConfig) {
            provider.appendTooltip((T) entityAccessor.getEntity(), new FabricTooltipHelper(iTooltip), entityAccessor.showDetails(), provider.requiresServerData() ? entityAccessor.getServerData() : null);
        }
    ...
    private record FabricEntityDataProvider<T extends Entity>(CommonEntityServerAppender<T> serverAppender) implements IServerDataProvider<EntityAccessor> {
    ...
    private record FabricTooltipHelper(ITooltip tooltip) implements CommonTooltipHelper {
        ...
        public void airBubbles(int breath, boolean bursting) {
            tooltip.add(new FabricAirBubbleElement(breath, bursting));
        }

        @Override
        public void appendMultilineText(Component... lines) {
            tooltip.append(new FabricMultiLineTextElement(lines));
        }
